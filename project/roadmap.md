...
| SEM-01C | **IN PROGRESS** | Revisit active Phi texts after the current module set and classification reviews are complete. | The [active text corpus review](../documents/evaluation/active_text_corpus_review.md) records completed reviews of all thirteen literary works and thirteen post-migration literary retrofits; *News from Nowhere* chapter 1 changes 29 of 85 units and uses six optional roots through four modules | Finish the literary shelf with *News from Nowhere* chapters 2 and 3 before auditing the book, manual, primer, pamphlets, and current documents. Preserve each work's declared method and every composition that remains the better Phi expression. |
...
| SEM-09 | **PARTIAL** | Migrate inherited vocabulary prose to the target contract and revisit the active corpus in context. | D017, the target JSON Schema, field-aware explorer, D052 with all 1,275 entries target-shaped, D053, and thirteen contextual literary retrofits | Complete SEM-09D. Change active Phi only where the present lexicon improves fidelity or naturalness. |
...
| SEM-09D | **IN PROGRESS** | Retrofit active Phi passages after the full vocabulary-prose migration. | Thirteen literary retrofits are complete through *News from Nowhere* chapter 1; its 85 source units remain exact while direct words repair authority, speech, coercion, affect, winter, wear, observation, waking, and difficulty | Finish with *News from Nowhere* chapters 2 and 3. Audit the book afterwards, followed by the manual, primer, pamphlets, and current documents. |
...
The three-syllable lexical migration, literary-shelf review, recovered vocabulary decisions, and all base and module prose migrations are complete. The contextual active-corpus retrofit has thirteen literary passes complete through *News from Nowhere* chapter 1. Only chapters 2 and 3 remain on the literary shelf, each kept as its own pull request while all three chapters provide continuity context. The broader sweep then starts with the book's cold open before turning to the manual, primer, pamphlets, and current documents; writing chapter 7 resumes afterward. A transparent composition stays when it remains better Phi.
