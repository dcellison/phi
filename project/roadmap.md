...
| SEM-01C | **IN PROGRESS** | Revisit active Phi texts after the current module set and classification reviews are complete. | The [active text corpus review](../documents/evaluation/active_text_corpus_review.md) records fifteen post-migration literary retrofits and the later source-fidelity repairs through D082; the [active teaching corpus review](../documents/evaluation/active_teaching_corpus_review.md) records all seven manual parts, the primer, and all nine pamphlets | Apply the same post-conversion fidelity sweep to *The Velveteen Rabbit*. Continue *The Prophet* in source order after the current shelf work. |
...
| ID | Status | Deliverable | Evidence | Completion or next criterion |
|---|---|---|---|---|
| COR-01 | **DONE** | A validated shelf of translations, transmutations, four original Phi works, and extensive teaching examples; each source-led work declares one method. | `texts/`; manual; primer | Keep every work labelled by method and continue to distinguish source fidelity, deliberate transmutation, and original Phi composition. |
...
| PUB-06 | **DONE** | Give each website shelf a renderer-led visual identity while keeping its Markdown clean. | The renderer gives the book, primer, texts, manual, and pamphlets distinct identities without moving presentation into their Markdown. Translation pages have individual motifs, Tengwar titles, exact interlinear source rows, continuous Phi readings, and translation limits; D079 brings the six formerly paired works into that form. *The Prophet* has an author-collection form. Standalone transmutations, original Phi work, and *News from Nowhere* retain forms suited to their own reading structures, while the contents page shows only methods represented on the current shelf. The manual has part and reading navigation under its maintained editorial manifest. Every pamphlet uses its workbook motif, navigation, exercises, and responsive reading treatment, and the catalogue keeps the maintained shelf order. The site build pins each treated source shape and stops when prose structure drifts from its renderer contract. | Complete. |
