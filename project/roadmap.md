...
| SEM-01C | **PENDING** | Revisit active Phi texts after the current module set and classification reviews are complete. | The [active text corpus review](../documents/evaluation/active_text_corpus_review.md) records completed reviews of all thirteen literary works: nine close translations and eleven transmuted works; D044 completes the base queue | Begin after SEM-09C leaves no legacy vocabulary prose, then audit active examples and passages across the book, manual, primer, pamphlets, texts, and current documents for workarounds that expanded vocabulary can replace more faithfully. Preserve translation and transmutation method, historical before-state evidence, and any composition that remains the better Phi expression. |
...
| SEM-09A | **DONE** | Apply a semantic coverage gate to content prose batches and preserve every noticed lexical decision. | `project/content_vocabulary_coverage.md` audits every completed batch; `project/content_vocabulary_decisions.json` has no open or accepted candidate; D020 through D049 record the resulting additions, compositions, deferrals, source boundaries, and refusals. | Keep the gate active during module migration so a newly noticed question receives an ID before its batch closes. |
...
| SEM-09C | **PARTIAL** | Complete target-prose migration for module content vocabulary. | D045 completes Philosophical Reasoning, D046 Systems and Shared Infrastructure, D047 Ecological Systems and Material Life, D048 Commons and Collective Governance, and D049 Household and Daily Life; 57 unique module entries remain legacy-shaped | Continue with Medical and Bodily Care, preserve current module memberships, and run the same prose, voice, coverage, and validation gates on every batch. |
| SEM-09D | **PENDING** | Retrofit active Phi passages after the full vocabulary-prose migration. | SEM-01C and the completed literary-shelf review supply the method; D020 through D049 and the remaining module passes will supply the vocabulary to inspect | Begin only after SEM-09C reaches zero legacy entries, then sweep the book, manual, primer, pamphlets, texts, and current documents; replace an older workaround only when the current word is more faithful or natural in context, and validate every changed Phi passage. |
...
| PED-08 | **PENDING** | Continue the Phi book under its treatment and established chapter voice. | `book/treatment.md`; completed chapters in `book/` | Resume the next planned chapter after SEM-09C and the subsequent SEM-09D corpus retrofit, then audit its citations before publication. |
...
The three-syllable lexical migration, literary-shelf review, recovered vocabulary decisions, and base prose migration are complete. The first five historical module prose batches are also finished through Household and Daily Life. The active sequence is now fixed: finish the remaining 57 module entries, sweep active Phi passages for workarounds that the completed lexicon can improve, then return to the Phi book. Keeping the remaining vocabulary migration together protects a consistent editorial voice across the lexicon. The later sweep is contextual rather than mechanical: a transparent composition stays when it remains better Phi.
...
2. Continue through the remaining 57 module entries. Medical and Bodily Care comes next, followed by Accessibility and Participation and Work, Craft, and Repair. Preserve every established module membership and apply the prose, voice, content-coverage, and validation gates to each batch.
3. After the module queue reaches zero, inspect active Phi in the book, manual, primer, pamphlets, texts, and current documents. Replace an older workaround only when the completed vocabulary improves fidelity or naturalness in that exact passage.
