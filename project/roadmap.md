...
| SEM-01C | **IN PROGRESS** | Revisit active Phi texts after the current module set and classification reviews are complete. | The [active text corpus review](../documents/evaluation/active_text_corpus_review.md) records completed reviews of all thirteen literary works and eleven post-migration literary retrofits; the Ring Verse keeps its base-only refusal while the surrounding analysis now uses completed Commons distinctions | Continue with *The Velveteen Rabbit* and *News from Nowhere* chapters 1-3, in that order, before auditing the book, manual, primer, pamphlets, and current documents. Preserve each work's declared method and every composition that remains the better Phi expression. |
...
| SEM-09 | **PARTIAL** | Migrate inherited vocabulary prose to the target contract and revisit the active corpus in context. | D017, the target JSON Schema, field-aware explorer, D052 with all 1,275 entries target-shaped, D053, and eleven contextual literary retrofits | Complete SEM-09D. Change active Phi only where the present lexicon improves fidelity or naturalness. |
...
| SEM-09D | **IN PROGRESS** | Retrofit active Phi passages after the full vocabulary-prose migration. | Eleven literary retrofits are complete through the Ring Verse refusal; all 12 forms survive, and the revised explanation keeps rule, enforcement, authority, control, coercion, and bond distinct | Continue with *The Velveteen Rabbit* and *News from Nowhere* chapters 1-3. Audit the book afterwards, followed by the manual, primer, pamphlets, and current documents. |
...
The three-syllable lexical migration, literary-shelf review, recovered vocabulary decisions, and all base and module prose migrations are complete. The contextual active-corpus retrofit is in progress, with eleven literary passes complete through the Ring Verse refusal. *The Velveteen Rabbit* and *News from Nowhere* chapters 1-3 remain, with the Morris chapters held for last. The broader sweep then starts with the book's cold open before turning to the manual, primer, pamphlets, and current documents; writing chapter 7 resumes afterward. A transparent composition stays when it remains better Phi.
