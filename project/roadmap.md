...
| SEM-09 | **PARTIAL** | Migrate inherited vocabulary prose to the target contract and revisit the active corpus in context. | D017, the target JSON Schema, field-aware explorer, D052 with all 1,275 entries target-shaped, D053, and the first two contextual literary retrofits | Complete SEM-09D. Change active Phi only where the present lexicon improves fidelity or naturalness. |
...
| SEM-09D | **IN PROGRESS** | Retrofit active Phi passages after the full vocabulary-prose migration. | Schleicher's paired renderings survive a complete base-and-module check; D053 repairs the animal range of `kawhera` and `pilora`; *The North Wind and the Sun* adopts `mesatu`, `komeri`, and `phesari` in both renderings | Continue through the text shelf one work at a time, with the Metta Sutta next, then sweep the book, manual, primer, pamphlets, and current documents. Replace an older workaround only when the current word is more faithful or natural in context. |
...
The three-syllable lexical migration, literary-shelf review, recovered vocabulary decisions, and all base and module prose migrations are complete. The contextual active-corpus retrofit is in progress. Schleicher's fable and *The North Wind and the Sun* are complete, and the Metta Sutta is next; the broader corpus sweep and the return to the Phi book follow. A transparent composition stays when it remains better Phi.
