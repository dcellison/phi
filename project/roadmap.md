...
| Evidence-led semantic growth | **PARTIAL** | All eight active profiles have completed first vocabulary passes, every recovered content-vocabulary question has an explicit decision, and all 1,299 lexicon entries follow the target prose contract. The legacy corpus retrofit is complete, while the stricter paired-text fidelity sweep is current through Gibran's On Love. The Metta Sutta repair adds integrity, virtue, pleasure, and craving under D068; Article 1 adds reason as a faculty under D069; Babel adds Work `torali` tower under D070; The Little Prince and Ring Verse repairs use existing vocabulary under D071 and D072; The Velveteen Rabbit adds base `tinemu` ant under D073; Schleicher's fable adds shared Ecological and Work `mophiku` shear under D074; On Giving adds shared `parelu` deserve and `thaweno` reward under D075; On Work repairs nine fidelity classes without adding a root under D076; On Love adds base `ratenu` steep and `salenu` unclothed while repairing nineteen close units and nine transmutation units under D077. |
...
|---|---|---|---|---|
| FND-01 | **DONE** | Classify Phi as a philosophy-first artlang/engineered contemplative practice, not an auxlang. | `documents/evaluation/language_assessment.md`; `project/development_log.md` D001 | Revisit only if actual use changes the project's purpose. |
...
| EXT-03 | **DONE** | Establish source-material separation and ethical reporting policy. | Canon ruling and `documents/design/word_shape_and_external_boundaries.md` | Phi may point to, translate, and analyze a separately preserved source without importing its grammar or claiming to exhaust it. |
...
| EXT-06 | **OPTIONAL** | Human choice exercise across core vocabulary, modules, transparent expressions, productive names, translation, and separate source presentation. | `documents/evaluation/solo_evaluation_tasks.md` section 2 | Record cases where the intended boundary or referent remains unclear and revise teaching where useful. |
...
| ID | Status | Deliverable | Evidence | Completion or next criterion |
|---|---|---|---|---|
| PHL-01 | **DONE** | Capability matrix with demonstrated, compositional, external-bridge, and gap classifications. | `documents/evaluation/philosophical_capability_matrix.md` | Use the matrix as a descriptive reference rather than an admission gate. |
...
| EVA-02 | **OPTIONAL** | Source-material and name-choice prompt set. | Protocol in `documents/evaluation/solo_evaluation_tasks.md` | Use the prompts when they would improve teaching or reveal a practical burden; no delay is required. |
...
| EVA-08 | **READY** | Consolidated evaluation record. | Result table in `documents/evaluation/solo_evaluation_tasks.md` | Add dated results, material, failures, and follow-up; never replace a pending row with an undocumented impression. |
...
| SPH-03 | **DONE** | Rename gate requiring repeated observed confusion rather than distance alone. | `documents/evaluation/listening_audit.md` | No discretionary or collision-driven rename bypasses the gate; an independently accepted structural migration records and validates its separate reason. |
...
| SEM-01C | **IN PROGRESS** | Revisit active Phi texts after the current module set and classification reviews are complete. | The [active text corpus review](../documents/evaluation/active_text_corpus_review.md) records fifteen post-migration literary retrofits and the later source-fidelity repairs through D077; the [active teaching corpus review](../documents/evaluation/active_teaching_corpus_review.md) records all seven manual parts, the primer, and all nine pamphlets | The four Gibran selections are current. Choose the next source-led fidelity task deliberately, preserving each work's declared method and every composition that remains the better Phi expression. |
...
| SEM-07 | **DONE** | First philosophical vocabulary expansion. | 18 Philosophical Reasoning module words, four base words established with the pass, nine later source-led additions, and Part VII teaching chapter | Continue through original writing and future vocabulary passes; no delayed evaluation prerequisite remains. |
...
| SEM-07B | **DONE** | First ecological systems and material life vocabulary expansion. | The original 27 module words, three base words, and D047 parameter nouns for temperature, humidity, and illumination; retained compositions, generated index, and Part VII teaching chapter | Use the module in original ecological and material writing and revise any term or composition that feels inadequate. |
| SEM-07C | **DONE** | First commons and collective governance vocabulary expansion. | The original 27 module words, three base words, D048 additions for governance and petition, and shared `parelu` deserve; retained compositions, generated index, and Part VII teaching chapter | Use the module in original governance and institutional writing and revise any term or composition that feels inadequate. |
| SEM-07D | **DONE** | First work, craft, and repair vocabulary expansion. | 28 Work, Craft, and Repair module words, two base words established with the pass, later source-led `torali` tower, `mophiku` shear, and `thaweno` reward, retained compositions and source boundaries, generated index, and Part VII teaching chapter | Use the module in original work, learning, repair, and labor writing and revise any term or composition that feels inadequate. |
...
| SEM-09 | **DONE** | Migrate inherited vocabulary prose to the target contract and revisit the active corpus in context. | D017, the target JSON Schema, field-aware explorer, D052 with every entry target-shaped, D053, fifteen contextual literary retrofits, the book review, all seven manual parts, the primer, all nine pamphlets, and the current-documents sweep | Reopen a completed passage only for a real semantic, factual, voice, or corpus problem. |
...
| SEM-09D | **DONE** | Retrofit active Phi passages after the full vocabulary-prose migration. | Fifteen literary retrofits complete the shelf, the Phi book is current, all seven manual parts, the primer, and all nine pamphlets are reviewed, and the current grammar, reference, design, module, vocabulary, and evaluation documents complete the sweep | Reopen a completed passage only for a real semantic, factual, voice, or corpus problem. |
...
5. Fill the target schema fields accurately. Supply natural structured examples and appropriate semantic domains or module membership.
...
| ID | Status | Deliverable | Evidence | Completion or next criterion |
|---|---|---|---|---|
| COR-01 | **DONE** | A validated shelf of ten close translations, eleven transmuted works, four original Phi works, and extensive teaching examples; eight source works pair the two methods. | `texts/`; manual; primer | Keep every work labeled by method and continue to distinguish source fidelity, deliberate transmutation, and original Phi composition. |
...
| PUB-06 | **DONE** | Give each website shelf a renderer-led visual identity while keeping its Markdown clean. | The renderer gives the book and complete primer distinct identities without changing their source. The text shelf is also complete. Six regular paired texts use the anthology design, with individual motifs, Tengwar titles, exact interlinear rows, responsive comparison ledgers, and strict source-shape checks. *The Prophet* adds a collection form for its four teachings. The translation-only form gives the Metta Sutta numbered verses and a quiet recitation, and gives the Solarpunk Manifesto stronger proposition divisions. Each ends by naming what Phi could not carry exactly from its source. The standalone form keeps Tolkien's couplet apart for examination before the Ring refusal, leads through the three *Little Prince* excerpts, and maps all eighteen scenes of *The Velveteen Rabbit* across 427 source-and-Phi rows. The renderer decodes quoted source witnesses before display, so backslash escapes stay off the page. The original form gives each of its 39 dialogue units a speaker gutter with Phi first and verifies every row in the three analytical ledgers. The contents page explains all four source relationships and carries thirteen short works in catalogue order with Phi, English, and Tengwar titles. *News from Nowhere* has a separate book identity, progress catalogue, chapter mastheads, and all 35 movements mapped across 686 source-and-Phi passages. Its strict record catches source-shape drift, while its own reader memory keeps a long book from displacing the reader's place elsewhere on the shelf. The manual now opens with a catalogue of seven parts, 24 chapters, and 145 readings. Part jumps and global reading numbers make the long reference navigable, while the established marks distinguish the sequence through its closing pages. The same manifest supplies correct names to every page header and must agree with the compact repository outline. The reading sources remain unchanged; the stale README and outline that repeated page titles now describe the present structure. The full matrix covers 292 wide and narrow loads, with separate dark, print, glossary, and colophon checks. *The Three Slots* establishes the practice layout on each of its thirteen pages. Adverbial, complementizer, coordination, and relative-clause practice adds another 38 under a paired-bracket motif. Time and evidence add 23 more pages, each with its own mark: a clock and focus for event views, a route and eye for claimed source. The two reference workbooks add another 23 pages under their own participant and noun-phrase marks. Naming and Source Material add 21 pages under distinct marks. Their source shapes add intentional sectionless lessons, a reasoning page, and a table-first reference leaf to the workbook renderer. Spoken Punctuation and Tengwar Mode add 25 pages under separate sound and writing marks. Tengwar fences become semantic figures with glyphs, romanized Phi, glosses, and translations where the source has them. Its compact lessons stay unsectioned, the exercise labels recover their hierarchy, and the final page reads as a twenty-two-line copybook. Counting in Phi adds the final eleven source pages under an abacus-and-scale mark. Its small-number correspondence is a two-column drill; the higher-number reading set is a numbered recital. The market exchange remains interlinear despite its spoken dash, and inline answer labels recover ten navigable groups. Rendered adaptation still handles the complementizer key, exact contents punctuation, split interlinear glosses, and consecutive answers collapsed by Markdown. It also restores Naming's paragraph-labelled answer parts and Tengwar's bold exercise divisions. The coverage record fixes the shape of all 154 pamphlet pages and every non-default list start. The 462-load matrix checks wide, narrow, and stylesheet-free output. Direct inspection ranged from the opening and core lessons to Market Day. Error, answer-key, and appendix layouts had separate desktop and phone checks. The pamphlet catalogue completes the sequence. It keeps all fourteen workbooks in catalogue order, counts their 154 reading pages from source, and gives each entry the motif used inside its workbook. Wide screens use two columns; a 390-pixel frame uses one without overlap or horizontal overflow. The semantic ordered list remains readable without CSS and under print styling. | Complete. |
| PUB-07 | **DONE** | Give Kia, the Short Road, and the Phi Book contents page distinct entrance treatments without decorating their Markdown. | Kia has its light threshold. Its Romanized and Tengwar greetings frame six numbered encounters and seven linked destinations, each open across its full row. The Short Road is a twelve-stop walk with a compact map, joined waypoints, thirteen interlinear figures, and a closing invitation in both writing systems. The Phi Book contents page is a map of the completed argument. A cover-like opening leads into four movements across chapters 1 through 14. From the boatman at the river it runs to the final door; the bibliography follows outside that route. The source list and published book must agree exactly. All three layouts hold in both site themes, at phone width, in print, and without a stylesheet. | Complete. |
...
**Roadmap items:** SEM-09B, SEM-09C, and SEM-09D.
