...
| PUB-06 | **IN PROGRESS** | Give each website shelf a renderer-led visual identity while keeping its Markdown clean. | The renderer gives the book and complete primer distinct identities without changing their source. *The North Wind and the Sun* adds the first texts prototype, with method-aware sections, structured interlinear passages, responsive comparison ledgers, and a Tengwar title. | Review the paired-text prototype, then extend the anthology design across the remaining forms and the contents page. Give *News from Nowhere* its book-level treatment within the shelf, and keep the book's inline treatment for current Phi inside English paragraphs as a shared convention. |
