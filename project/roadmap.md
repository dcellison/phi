...
| SEM-01C | **NEXT** | Revisit active Phi texts after the current module set and classification reviews are complete. | The [active text corpus review](../documents/evaluation/active_text_corpus_review.md) records completed reviews of all thirteen literary works: nine close translations and eleven transmuted works; D044 completes the base queue | Audit active examples and passages across the book, manual, primer, pamphlets, texts, and current documents for workarounds that a new base word can replace more faithfully. Preserve translation and transmutation method, historical before-state evidence, and any composition that remains the better Phi expression. |
...
| SEM-09D | **NEXT** | Retrofit active Phi passages after the base-vocabulary migration. | SEM-01C and the completed literary-shelf review supply the method; D020 through D044 supply the new base vocabulary to inspect | Sweep the book, manual, primer, pamphlets, texts, and current documents; replace an older workaround only when the new word is more faithful or natural in context, then validate every changed Phi passage. |
| SEM-09C | **PENDING** | Complete target-prose migration for module content vocabulary. | 209 module entries remain legacy-shaped after D044 | Begin after the SEM-09D corpus retrofit, preserve current module memberships, and run the same coverage gate on every batch. |
...
| PED-08 | **PENDING** | Continue the Phi book under its treatment and established chapter voice. | `book/treatment.md`; completed chapters in `book/` | Resume the next planned chapter after the SEM-09D corpus retrofit and SEM-09C, then audit its citations before publication. |
...
2. Inspect active Phi in the book, manual, primer, pamphlets, texts, and current documents. Replace an older workaround only when the new base word improves fidelity or naturalness in that exact passage.
3. Continue through module entries after the corpus retrofit. Preserve every established module membership and apply the content-coverage gate to each batch.
