...
| SEM-09A | **NEXT** | Apply a semantic coverage gate to content prose batches and resolve the retrospective review queue. | `project/content_vocabulary_coverage.md` audits the six completed base content groups; D020 and four target-shaped entries establish damage, wear, weakness, and rigidity in base vocabulary; the first practical scenario recommends a base adjective for adhesion. | Decide the adhesion recommendation, then test traction, material phase, and neutral measurement; record each decision before resuming inherited content prose migration. |
