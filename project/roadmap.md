...
| SEM-09A | **IN PROGRESS** | Apply a semantic coverage gate to content prose batches and preserve every noticed lexical decision. | `project/content_vocabulary_coverage.md` audits every completed base batch; `project/content_vocabulary_decisions.json` separates migration state from lexical closure and CI rejects unregistered review rows or a batch closed over open work. D020 through D031 record accepted additions. | Decide the fourteen remaining open candidate groups, then continue inherited migration without allowing prose completion to erase or defer an unrecorded lexical question. |
