...
| SEM-01C | **IN PROGRESS** | Revisit active Phi texts after the current module set and classification reviews are complete. | The [active text corpus review](../documents/evaluation/active_text_corpus_review.md) records completed reviews of all thirteen literary works, fifteen post-migration literary retrofits, and the complete drafted Phi book | Reassess the order of chapter 7 and the remaining manual, primer, pamphlet, and current-document sweep. Preserve each work's declared method and every composition that remains the better Phi expression. |
...
| SEM-09 | **PARTIAL** | Migrate inherited vocabulary prose to the target contract and revisit the active corpus in context. | D017, the target JSON Schema, field-aware explorer, D052 with all 1,275 entries target-shaped, D053, fifteen contextual literary retrofits, and the drafted-book review | Complete SEM-09D. Change active Phi only where the present lexicon improves fidelity or naturalness. |
...
| SEM-09D | **IN PROGRESS** | Retrofit active Phi passages after the full vocabulary-prose migration. | Fifteen literary retrofits complete the shelf, and the drafted Phi book is current; the book pass repairs tide, service, quantity, method, and inherited schema claims without adding a root | Reassess the execution sequence, then continue through the remaining manual, primer, pamphlets, and current documents when that work returns to the front. |
...
The three-syllable lexical migration, literary-shelf review, recovered vocabulary decisions, and all base and module prose migrations are complete. Fifteen contextual literary passes complete the active shelf, and the drafted Phi book is current. The requested reassessment now decides whether chapter 7 precedes the remaining manual, primer, pamphlet, and current-document sweep. A transparent composition stays when it remains better Phi.
...
3. The literary shelf and drafted book are current. Inspect active Phi in the manual, primer, pamphlets, and current documents when the remaining sweep resumes. Replace an older workaround only when the completed vocabulary improves fidelity or naturalness in that exact passage.
