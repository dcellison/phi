...
| SEM-01C | **READY** | Revisit active Phi texts after the current module set and classification reviews are complete. | The [active text corpus review](../documents/evaluation/active_text_corpus_review.md) records completed reviews of all thirteen literary works: nine close translations and eleven transmuted works; D052 leaves the full lexicon target-shaped | Proceed within SEM-09D by auditing active examples and passages across the book, manual, primer, pamphlets, texts, and current documents for workarounds that expanded vocabulary can replace more faithfully. Preserve translation and transmutation method, historical before-state evidence, and any composition that remains the better Phi expression. |
...
| SEM-09 | **PARTIAL** | Migrate inherited vocabulary prose to the target contract and revisit the active corpus in context. | D017, the target JSON Schema, field-aware explorer, committed coverage report, and D052 with all 1,275 entries target-shaped | Complete the contextual retrofit in SEM-09D. Change active Phi only where the present lexicon improves fidelity or naturalness. |
...
| SEM-09D | **NEXT** | Retrofit active Phi passages after the full vocabulary-prose migration. | SEM-01C and the completed literary-shelf review supply the method; D020 through D052 supply the vocabulary to inspect | Sweep the book, manual, primer, pamphlets, texts, and current documents; replace an older workaround only when the current word is more faithful or natural in context, and validate every changed Phi passage. |
...
The three-syllable lexical migration, literary-shelf review, recovered vocabulary decisions, and all base and module prose migrations are complete. The active sequence is now fixed: sweep active Phi passages for workarounds that the completed lexicon can improve, then return to the Phi book. The sweep is contextual rather than mechanical: a transparent composition stays when it remains better Phi.
