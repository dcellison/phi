...
| Evidence-led semantic growth | **PARTIAL** | All eight active profiles have completed first vocabulary passes, every recovered content-vocabulary question has an explicit decision, and all 1,275 lexicon entries follow the target prose contract. The contextual active-corpus retrofit is next. |
...
| SEM-01C | **IN PROGRESS** | Revisit active Phi texts after the current module set and classification reviews are complete. | The [active text corpus review](../documents/evaluation/active_text_corpus_review.md) records completed reviews of all thirteen literary works and seven post-migration retrofits; D053 repairs two supporting power verbs, later base roots improve the North Wind and Metta passages, UDHR Article 1 preserves both renderings, Babel and the Heart Sutra repair contextual choices, and *The Prophet, On Children* restores three lexical distinctions | Continue within SEM-09D through the literary shelf, then audit active examples and passages across the book, manual, primer, pamphlets, and current documents. Preserve translation and transmutation method, historical before-state evidence, and any composition that remains the better Phi expression. |
...
| SEM-09 | **PARTIAL** | Migrate inherited vocabulary prose to the target contract and revisit the active corpus in context. | D017, the target JSON Schema, field-aware explorer, D052 with all 1,275 entries target-shaped, D053, and the first seven contextual literary retrofits | Complete SEM-09D. Change active Phi only where the present lexicon improves fidelity or naturalness. |
...
| SEM-09D | **IN PROGRESS** | Retrofit active Phi passages after the full vocabulary-prose migration. | Schleicher's paired renderings survive a complete base-and-module check; D053 repairs the animal range of `kawhera` and `pilora`; the North Wind and Metta passages adopt later base roots; UDHR Article 1 preserves both renderings; Babel and the Heart Sutra repair contextual choices; *The Prophet, On Children* restores offspring, Life, and trying | Continue through the text shelf one work at a time, with the Tao Te Ching selections next, then sweep the book, manual, primer, pamphlets, and current documents. Replace an older workaround only when the current word is more faithful or natural in context. |
...
The three-syllable lexical migration, literary-shelf review, recovered vocabulary decisions, and all base and module prose migrations are complete. The contextual active-corpus retrofit is in progress. Schleicher's fable, *The North Wind and the Sun*, the Metta Sutta, UDHR Article 1, Babel, the Heart Sutra, and *The Prophet, On Children* are complete, and the Tao Te Ching selections are next; the broader corpus sweep and the return to the Phi book follow. A transparent composition stays when it remains better Phi.
...
3. Inspect active Phi in the book, manual, primer, pamphlets, texts, and current documents. Replace an older workaround only when the completed vocabulary improves fidelity or naturalness in that exact passage.
