...
| SEM-09A | **NEXT** | Apply a semantic coverage gate to content prose batches and resolve the retrospective review queue. | `project/content_vocabulary_coverage.md` audits the six completed base content groups; D020 establishes damage, wear, weakness, and rigidity; D021 and base `shumeko` close adhesion; D022 and base `selawi` close traction; D023 and base `patoku`, `larewu`, and `heshowa` close ordinary material phase; the neutral-measurement scenario recommends coordinated base nouns for overall size, spatial length, and distance. | Decide the three-noun recommendation; if accepted, implement the set together, then resume inherited content prose migration. |
