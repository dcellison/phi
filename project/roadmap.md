...
| SEM-01C | **IN PROGRESS** | Revisit active Phi texts after the current module set and classification reviews are complete. | The [active text corpus review](../documents/evaluation/active_text_corpus_review.md) records completed reviews of all thirteen literary works and fifteen post-migration literary retrofits; *News from Nowhere* chapter 3 changes 46 of 245 units and uses 14 optional roots with 15 memberships across five modules | Audit the book from its cold open, followed by the manual, primer, pamphlets, and current documents. Preserve each work's declared method and every composition that remains the better Phi expression. |
...
| SEM-09 | **PARTIAL** | Migrate inherited vocabulary prose to the target contract and revisit the active corpus in context. | D017, the target JSON Schema, field-aware explorer, D052 with all 1,275 entries target-shaped, D053, and fifteen contextual literary retrofits | Complete SEM-09D. Change active Phi only where the present lexicon improves fidelity or naturalness. |
...
| SEM-09D | **IN PROGRESS** | Retrofit active Phi passages after the full vocabulary-prose migration. | Fifteen literary retrofits complete the shelf; *News from Nowhere* chapter 3 keeps all 245 source units exact while direct words repair rooms, meals, recognition, bodily detail, responses, and craft | Begin the book at its cold open. Follow it with the manual, primer, pamphlets, and current documents. |
