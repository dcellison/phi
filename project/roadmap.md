...
| PUB-06 | **IN PROGRESS** | Give each website shelf a renderer-led visual identity while keeping its Markdown clean. | The renderer gives the book and complete primer distinct identities without changing their source. Six regular paired texts now use the anthology design, with individual motifs, Tengwar titles, exact interlinear rows, responsive comparison ledgers, and strict source-shape checks. | Handle *The Prophet* as the mixed-form exception, then continue through standalone methods, original work, and the contents page. Give *News from Nowhere* its book-level treatment within the shelf, and keep the book's inline treatment for current Phi inside English paragraphs as a shared convention. |
