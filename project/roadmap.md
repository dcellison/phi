...
| PUB-06 | **READY** | Give each website shelf a renderer-led visual identity while keeping its Markdown clean. | The completed book treatment proves that site furniture can be layered over unchanged source; the ordered design brief lives in `project/deferred_questions.md`. | Prototype the primer on chapter 1, then continue through texts, manual, and pamphlets. Keep the book's inline treatment for current Phi inside English paragraphs as a shared convention. |
