...
| PUB-06 | **IN PROGRESS** | Give each website shelf a renderer-led visual identity while keeping its Markdown clean. | The renderer gives the book and complete primer distinct identities without changing their source. The text shelf is also complete. Six regular paired texts use the anthology design, with individual motifs, Tengwar titles, exact interlinear rows, responsive comparison ledgers, and strict source-shape checks. *The Prophet* adds a collection form for its three teachings. The translation-only form gives the Metta Sutta numbered verses and a quiet recitation, and gives the Solarpunk Manifesto stronger proposition divisions. Each ends by naming what Phi could not carry exactly from its source. The standalone form keeps Tolkien's couplet apart for examination before the Ring refusal, leads through the three *Little Prince* excerpts, and maps all eighteen scenes of *The Velveteen Rabbit* across 428 source-and-Phi rows. The renderer decodes quoted source witnesses before display, keeping backslash escapes off the page. The original form gives each of its 39 dialogue units a speaker gutter with Phi first and verifies every row in the three analytical ledgers. The contents page explains all four source relationships and carries thirteen short works in catalogue order with Phi, English, and Tengwar titles. *News from Nowhere* has a separate book identity, progress catalogue, chapter mastheads, and all 29 movements mapped across 565 source-and-Phi passages. Its strict record catches source-shape drift, while its own reader memory keeps a long book from displacing the reader's place elsewhere on the shelf. The manual's first 39 pages now share the approved reference design. First Light, Soul, and Phonology have separate marks; orientation, numbered divisions, and inline Phi remain common. The renderer includes continuous essays, H3 landmarks, one speaker-row variant, responsive reference tables, interlinear blocks, and exercise sections. A structural manifest records the heading, table, fence, quotation, list, and rule counts used by each transformation without pinning its prose. The browser matrix totals 78 wide and narrow page loads, then samples the intermediate phone width, dark palette, print media, and the unchanged grammar neighbour. The Part IV noun-phrase prototype also retains all six sections, three tables, two examples, and seven pattern positions. | Apply the design to Grammar, Complex Sentences, and Mastery. Follow with Reference and back matter, then finish the manual contents and cross-page review before starting the pamphlet prototype. |
