...
| SEM-01C | **IN PROGRESS** | Revisit active Phi texts after the current module set and classification reviews are complete. | The [active text corpus review](../documents/evaluation/active_text_corpus_review.md) records completed reviews of all thirteen literary works and eight post-migration literary retrofits; D053 repairs two supporting power verbs, later base roots improve the North Wind and Metta passages, UDHR Article 1 preserves both renderings, and Babel, the Heart Sutra, *The Prophet, On Children*, and the Tao Te Ching selections repair contextual choices | Continue within SEM-09D by auditing active examples and passages across the book, manual, primer, pamphlets, and current documents. Preserve translation and transmutation method, historical before-state evidence, and any composition that remains the better Phi expression. |
...
| SEM-09 | **PARTIAL** | Migrate inherited vocabulary prose to the target contract and revisit the active corpus in context. | D017, the target JSON Schema, field-aware explorer, D052 with all 1,275 entries target-shaped, D053, and eight contextual literary retrofits | Complete SEM-09D. Change active Phi only where the present lexicon improves fidelity or naturalness. |
...
| SEM-09D | **IN PROGRESS** | Retrofit active Phi passages after the full vocabulary-prose migration. | The eight-work literary phase is complete; D053 repairs the animal range of `kawhera` and `pilora`; later roots improve the North Wind, Metta, Babel, Heart Sutra, Prophet, and Tao passages; UDHR Article 1 preserves both renderings | Audit the book from its cold open through the drafted chapters, then continue through the manual, primer, pamphlets, and current documents. Replace an older workaround only when the current word is more faithful or natural in context. |
...
The three-syllable lexical migration, literary-shelf review, recovered vocabulary decisions, and all base and module prose migrations are complete. The contextual active-corpus retrofit is in progress. Its eight-work literary phase is complete through the Tao Te Ching selections. The broader sweep starts with the book's cold open and follows the drafted chapters before turning to the manual, primer, pamphlets, and current documents; writing chapter 7 resumes afterward. A transparent composition stays when it remains better Phi.
