...
| PUB-06 | **IN PROGRESS** | Give each website shelf a renderer-led visual identity while keeping its Markdown clean. | The renderer gives the book and complete primer distinct identities without changing their source. Six regular paired texts use the anthology design, with individual motifs, Tengwar titles, exact interlinear rows, responsive comparison ledgers, and strict source-shape checks. *The Prophet* adds a collection form for its three teachings. The translation-only form gives the Metta Sutta numbered verses and a quiet recitation, and gives the Solarpunk Manifesto stronger proposition divisions. Each ends by naming what Phi could not carry exactly from its source. The standalone form keeps Tolkien's couplet apart for examination before the Ring refusal, leads through the three *Little Prince* excerpts, and maps all eighteen scenes of *The Velveteen Rabbit* across 428 source-and-Phi rows. The renderer now decodes quoted source witnesses before display, keeping backslash escapes off the page. The original form gives each of its 39 dialogue units a speaker gutter with Phi first. It also verifies every row in the three analytical ledgers. The contents page explains all four source relationships and carries thirteen short works in catalogue order with Phi, English, and Tengwar titles. It sets the book apart with its current chapter count, while build-time checks reject incomplete catalogue coverage. | Give *News from Nowhere* its book-level treatment within the shelf. Preserve the book's inline treatment for current Phi inside English paragraphs as a shared convention. |
