...
| SEM-09A | **DONE** | Apply a semantic coverage gate to content prose batches and preserve every noticed lexical decision. | `project/content_vocabulary_coverage.md` audits every completed batch; `project/content_vocabulary_decisions.json` has no open or accepted candidate; D020 through D051 record the resulting additions, compositions, deferrals, source boundaries, and refusals. | Keep the gate active during module migration so a newly noticed question receives an ID before its batch closes. |
...
| SEM-09C | **PARTIAL** | Complete target-prose migration for module content vocabulary. | D045 through D051 complete seven learning paths through Accessibility and Participation; the final 16 unique module entries belong to Work, Craft, and Repair | Complete Work, Craft, and Repair while preserving current module memberships and applying the same prose, voice, coverage, and validation gates. |
| SEM-09D | **PENDING** | Retrofit active Phi passages after the full vocabulary-prose migration. | SEM-01C and the completed literary-shelf review supply the method; D020 through D051 and the final Work pass will supply the vocabulary to inspect | Begin only after SEM-09C reaches zero legacy entries, then sweep the book, manual, primer, pamphlets, texts, and current documents; replace an older workaround only when the current word is more faithful or natural in context, and validate every changed Phi passage. |
...
The three-syllable lexical migration, literary-shelf review, recovered vocabulary decisions, and base prose migration are complete. Seven historical module prose batches are also finished through Accessibility and Participation. The active sequence is now fixed: finish the final 16 Work, Craft, and Repair entries, sweep active Phi passages for workarounds that the completed lexicon can improve, then return to the Phi book. Keeping the remaining vocabulary migration together protects a consistent editorial voice across the lexicon. The later sweep is contextual rather than mechanical: a transparent composition stays when it remains better Phi.
...
2. Complete the final 16 module entries in Work, Craft, and Repair. Preserve every established module membership and apply the prose, voice, content-coverage, and validation gates to the batch.
