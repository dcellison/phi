...
The live thread is PED-08, the Phi book. Twelve files are drafted and merged: the cold open, chapters 1 through 10, and chapter 11, which was drafted early because it tests the book's hardest empirical claims. Chapters 8 and 9 have archived post-merge citation audits; chapter 10 was verified against primary sources before drafting, and its post-merge audit waits on the maintainer's go. What remains is Part IV, chapters 12 through 14, and the close, each drafted on its own branch and pull request, each begun only on the maintainer's go.

The immediate next steps, in order and each on the maintainer's go, are the chapter 10 citation audit and then chapter 12, "A language for the commons". The chapter method is settled: a verification agent pins every source with verbatim quotes before drafting, author names and title lines come from Crossref at entry-writing time, repository receipts are checked by grep rather than recall, the register follows the maintainer's hedging-low and warmth-high rulings recorded in [`language_and_voice.md`](language_and_voice.md), the full voice protocol runs with the counting pass, and an audit's own findings are verified before they are applied.
