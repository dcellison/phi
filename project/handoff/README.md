...
The immediate task is the next SEM-09D literary pass, [`texts/news_from_nowhere/chapter_02.md`](../../texts/news_from_nowhere/chapter_02.md), followed by chapter 3 in its own pull request. Treat the chapters as one work with internal dependencies. Chapter 1 now establishes `tiripe` for the moonlit river and distinguishes held views, alliance membership, and authority; later chapters must not drift back to the retired workarounds. Optional vocabulary may be used freely when it fits the scene, but it must not turn Morris's social relations into tidy modern abstractions or erase the narrator's misunderstandings. Change a passage only when current vocabulary makes it more faithful or natural. A transparent composition is not obsolete merely because a direct word now exists.
