...
1. Complete *News from Nowhere* chapters 1-3, the final literary retrofit.
...
The immediate task is the final SEM-09D literary pass through [`texts/news_from_nowhere/`](../../texts/news_from_nowhere/), beginning with chapter 1 and continuing through chapter 3. Treat the chapters as one work with internal dependencies. Optional vocabulary may be used freely when it fits the scene, but it must not turn Morris's social relations into tidy modern abstractions or erase the narrator's misunderstandings. Change a passage only when current vocabulary makes it more faithful or natural. A transparent composition is not obsolete merely because a direct word now exists.
