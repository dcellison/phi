...
The immediate task is the final literary pass, [`texts/news_from_nowhere/chapter_03.md`](../../texts/news_from_nowhere/chapter_03.md). Treat the three chapters as one work with internal dependencies. Chapter 1 establishes `tiripe` for the moonlit river and separates held views, alliance membership, and authority. Chapter 2 continues that river language, keeps the failed payment concrete, and gives present capacity to work through `pushali`. Chapter 3 must not drift back to the retired workarounds. Optional vocabulary may be used freely when it fits the scene, but it must not turn Morris's social relations into tidy modern abstractions or erase the narrator's misunderstandings. Change a passage only when current vocabulary makes it more faithful or natural. A transparent composition is not obsolete merely because a direct word now exists.
