...
- When a commission names an additive goal such as warmth, richness, or aspiration, track the additions per pass rather than only the corrections. Daniel called out a sweep whose later passes quietly shrank toward subtraction; declining effort across an arc is a finding to raise with him before he has to raise it.
