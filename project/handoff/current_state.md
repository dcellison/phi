...
This snapshot was first assembled after pull request #418 merged on 17 July 2026 and is current through pull request #622 on 28 July 2026. It covers D055 and the completed SEM-09D corpus retrofit. The finished book, the practice-complete pamphlet shelf, and the first five chapters of *News from Nowhere* are current here as well. It also records the writing-system consolidation that made Tengwar Phi's sole alternative to romanization, the four renderer-led shelf identities, and the complete entrance sequence for Kia, the Short Road, and the Phi Book contents page. Its counts describe the current tracked work; Git history remains the source for branch and merge identifiers.
...
The chapter uses the full vocabulary without adding a root or registering a compound. Direct `pilomu`, sycamore, replaced an early generic rendering during the vocabulary audit. Optional terms bring in `luseri` for provenance, `thesani` for skill, and `pherami` for dependency. Preservation uses `nelore`; objection uses `whakeru`. Exact source identities and values remain in Morris's witness. The focused corrective pass repaired modifier order and Slot 1 order. It restored purpose-first clauses and complete-event framing, removed one invalid passive-causative stack, and cut details that the back-translations had borrowed from the source without Phi support.
