...
| Shelf visual identities | In progress; primer and texts complete, manual reader treated through the colophon | The text shelf ends with *News from Nowhere* treated as one 32-chapter book. The manual's source Markdown is unchanged. Its working-reference frame now covers every one of the reader's 145 pages. Part VII uses an open book and bookmark. Each long module guide has a complete map, while its tables and examples still carry every source unit. Appendix A presents 28 linguistic definitions in two columns on wide screens and one on phones; its 67 abbreviation rows keep their hover and keyboard explanations. A quill and heart close the colophon, whose root and manual URLs share one rendered article. The latest browser matrix loaded every page at 1440 and 320 pixels for 290 checks. Separate probes covered dark mode, print, both colophon paths, and the glossary interaction. The sweep found two unlinked repository paths that the earlier linked-path rule could not wrap. Path-shaped code now receives the compact treatment without shrinking ordinary Phi. The contents treatment and the whole-manual consistency read come before the pamphlets. |
