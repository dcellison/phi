...
This snapshot was first assembled after pull request #418 merged on 17 July 2026 and has been refreshed through D055, the fifteenth SEM-09D literary retrofit, the drafted-book pass, chapter 7, all seven manual teaching passes, the primer pass, three pamphlet passes, and the Part I warmth revision. Its counts describe the current tracked work; Git history remains the source for branch and merge identifiers.
...
A warmth revision then re-tuned Part I's register without touching facts, Phi, glosses, or IPA. The recurring no-guarantee hedge now appears once per chapter at full strength and everywhere else converts to what each feature offers; the original manuscript's dwelling metaphor, founding question, and five-pillar portraits returned on design-intent verbs; and `documents/reference/voice_for_models.md` gained a waiver row capping hedge repetition in teaching prose, with a note that caution stamps as readily as flourish. Apply that budget in any future teaching-prose pass.
