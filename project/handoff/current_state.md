...
| Shelf visual identities | In progress; primer, texts, and manual complete, seven pamphlets complete | The pamphlet workbook frame now reaches 74 pages. Tense and aspect pairs its returning clock with focus brackets; evidentiality follows a source route to an eye. Each keeps its own accent while sharing the compact page rail, section labels, responsive tables, exercise panels, and print behaviour established by the earlier workbooks. A narrow contents rule reconciles `Witness: hi` with the page heading `Witness (hi)` without treating other wording changes as equivalent. The answer renderer restores exact consecutive runs that Markdown folded into a single item. The repair reaches the adverbial and coordination keys as well as the new tense key. Source shapes and list starts remain pinned. Rails contain no extractable text, motifs exist only in CSS, and simplified readers receive clean prose when the stylesheet is absent. The current browser matrix opens every treated page at 1440px, at 390px, and without CSS: 222 loads. The daytime palette was checked first. Night and print followed. The source journal, responsive tables, and answer keys received individual attention. The catalogue comes after the remaining seven. |
