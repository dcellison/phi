...
| Shelf visual identities | In progress; primer, texts, and manual complete, nine pamphlets complete | The pamphlet workbook frame now reaches 97 pages. The participant mark pairs a group with a dashed circle for the subject that can go unsaid. The noun-phrase mark pairs three kinds of shape with a branching list. Both keep the compact rail, responsive tables, exercise panels, and print behaviour established by the earlier workbooks. A narrow contents rule still reconciles `Witness: hi` with the page heading `Witness (hi)` and nothing broader. The answer renderer restores exact consecutive runs that Markdown folded into one item, including both new workbooks. Source shapes and list starts remain pinned. Rails contain no extractable text, motifs exist only in CSS, and simplified readers receive clean prose without the stylesheet. The current browser matrix opens every treated page at 1440px, at 390px, and without CSS: 291 loads. Light, dark, print, worked scenarios, responsive tables, and answer keys received direct checks. The remaining order is naming with source material, spoken punctuation with Tengwar mode, ternary numerals, then the shelf catalogue. |
