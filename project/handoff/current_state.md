...
This snapshot was first assembled after pull request #418 merged on 17 July 2026 and has been refreshed through D055, the fifteenth SEM-09D literary retrofit, the drafted-book pass, chapter 7, all seven manual teaching passes, the primer pass, five pamphlet passes, and the whole-manual warmth revision. Its counts describe the current tracked work; Git history remains the source for branch and merge identifiers.

## Active execution sequence

The literary shelf and Phi book are current through chapter 7. The execution sequence is now:

1. Resume SEM-09D with `pamphlets/relative_clauses/`, then continue through the other three pamphlets and the current documents.
...
SEM-09D is in progress. Fifteen literary passes complete the shelf, and all three drafted *News from Nowhere* chapters are among them. The Phi book is current through chapter 7. All seven manual parts, all twenty-seven primer files, and five pamphlets are reviewed without a lexical gap. Parts IV and VI each use one explicitly labelled optional term with a base alternative; Part VII teaches module words only inside their named learning paths, while the general primer and all five reviewed pamphlets use no module terms. The remaining active-corpus retrofit resumes with the other four pamphlets and current documents. A Phi passage changes only when the current form is more faithful or natural there. A transparent composition stays when its parts still teach or express the thought better, and every changed sentence and dependency receives validation.
