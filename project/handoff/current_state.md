...
| Shelf visual identities | In progress; primer and texts complete, manual Parts I through III complete | The text shelf ends with *News from Nowhere* treated as one 32-chapter book. Manual coverage now comprises 39 pages across the opening three parts, with source Markdown untouched. Their marks differ, but the orientation stays predictable. Essays without major divisions remain continuous; local navigation appears when a page has several numbered sections. The road exchange uses speaker lanes. At phone width, broad sound and drill tables reflow as field-and-value rows, while H3 anchors, exercise headings, and interlinear frames keep their place. The approved grammar prototype remains intact on the descriptors-and-quantifiers page. Shape signatures protect each renderer dependency. A headless sweep loaded the complete scope at 1440 and 320 pixels; separate probes covered the 390-pixel dialogue, dark palette, print restoration, table cells, title wrapping, and the untreated neighbouring grammar page. Extend the reference frame across Parts IV through VI next, then take in the reference part, appendices, and colophon. Close with the contents and a consistency pass before beginning the pamphlets. Keep current Phi inside English paragraphs visually consistent with the book's inline treatment. |
