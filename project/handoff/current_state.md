...
This snapshot was first assembled after pull request #418 merged on 17 July 2026 and has been refreshed through D055, the fifteenth SEM-09D literary retrofit, all seven manual teaching passes, the primer pass, all nine pamphlet passes, the whole-manual warmth revision, the current-documents sweep, the completed, published book, the whole-book revision read, the pamphlet-shelf survey with the expansion that completed the practice apparatus, the writing-system consolidation that made Tengwar Phi's sole alternative to romanization, and the renderer-led manual treatment through Part VI. Its counts describe the current tracked work; Git history remains the source for branch and merge identifiers.
...
| Shelf visual identities | In progress; primer and texts complete, manual Parts I through VI complete | The text shelf ends with *News from Nowhere* treated as one 32-chapter book. Manual coverage now comprises 128 teaching pages across Parts I through VI; the Markdown sources are unchanged. The opening marks remain distinct. Grammar carries ordered layers, Complex Structures joins a link with braces, and Mastery pairs a compass with a sprout. Ordinary prose and local maps share one dependable reading rhythm with speaker lanes and responsive tables. Exercises and interlinear frames join it without crowding the page. The later parts add grouped contrasts and case-study passages. Transmutation gets step headings; `rena` gets a syntax frame; the intentionally blank number table gets accessible labels from renderer metadata. Shape signatures protect every treated page and still pin the descriptors-and-quantifiers pattern. The two browser sweeps total 256 desktop-and-phone page loads, with further checks at an intermediate phone width, in dark mode, and under print media. The second sweep also caught a long repository path and a mixed English/Phi map label before either reached the branch. Extend the frame through Part VII, the appendices, and the colophon next. Build the contents page last, then read across the entire manual for consistency before beginning the pamphlets. Keep current Phi inside English paragraphs visually consistent with the book's inline treatment. |
