...
SEM-09D is in progress. Fifteen literary passes complete the shelf. All three drafted *News from Nowhere* chapters are among them. Active Phi in the book follows, from the cold open through the drafted chapters in file order; the manual, primer, pamphlets, and current documents come after it. A Phi passage changes only when the current form is more faithful or natural there. A transparent composition stays when its parts still teach or express the thought better, and every changed sentence and dependency receives validation.
...
The next planned draft is chapter 7, "A literature before a community." It comes after the remaining SEM-09D active-corpus sweep. Read [`book/treatment.md`](../../book/treatment.md), chapter 6, and several neighbouring finished chapters before drafting. Each chapter is its own PR. Audit every external citation after the draft, repair the prose from the audit, preserve the consistent `Phi sources` subsection, and use restrained Jerome humour where it fits.
