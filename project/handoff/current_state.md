...
This snapshot was first assembled after pull request #418 merged on 17 July 2026 and has been refreshed through D053 and the ninth SEM-09D literary retrofit. Its counts describe the current tracked work; Git history remains the source for branch and merge identifiers.

## Active execution sequence

The roadmap's current sequence is not an invitation to choose among several equal tasks. It records one active path:

1. SEM-09D: complete the remaining literary passes in order: *The Little Prince* excerpts, the Ring Verse refusal, *The Velveteen Rabbit*, then *News from Nowhere* chapters 1-3.
