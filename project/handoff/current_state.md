...
| Shelf visual identities | In progress; primer, texts, and manual complete, eleven pamphlets complete | The pamphlet workbook frame now reaches 118 pages. The new marks show a tagged person for Naming and a document beside a split panel for Source Material. The renderer now accepts deliberately unsectioned lessons, a separate reasoning-page treatment, and a table-first reference leaf without inventing source structure. Naming's six answer divisions regain proper headings. The style pass leaves exact artifacts as ordinary code and reserves the established inline treatment for canonical Phi. Source shapes and list starts remain pinned. Rails contain no extractable text, motifs exist only in CSS, and simplified readers receive clean prose without the stylesheet. The current browser matrix opens every treated page at 1440px, at 390px, and without CSS: 354 loads. Dark and print received separate checks. Direct inspection covered the household scenes and compact lessons; the reasoning, reference, and answer pages did too. Spoken punctuation with Tengwar mode comes next. Ternary numerals and the shelf catalogue close the sequence. |
