...
SEM-09D is in progress. The sweep begins with the text shelf, one work per pull request, before moving through the book, manual, primer, pamphlets, and current documents. Schleicher's fable, *The North Wind and the Sun*, the Metta Sutta, UDHR Article 1, Babel, the Heart Sutra, and *The Prophet, On Children* are complete; the Tao Te Ching selections are next. A Phi passage changes only when the current form is more faithful or natural there. A transparent composition stays when its parts still teach or express the thought better, and every changed sentence and dependency receives validation.
