...
| Shelf visual identities | In progress; primer, texts, and manual complete, five pamphlets complete | *The Three Slots* supplies the workbook frame across thirteen readings, and the clause family brings the treated pamphlet count to 51 pages. Its paired brackets and joining line suit pages about frames, embeddings, coordination, and noun descriptions while leaving their source structures intact. The renderer accepts numbered and unnumbered legacy titles. It follows either `##` or `###` as the primary lesson level, repairs the early complementizer answer-key hierarchy in rendered HTML, and separates its interlinear glosses without changing the Markdown. Long box-drawing examples remain inside local horizontal scrollers on phones. The source-shape record checks every page and remembers ordered-list starts. The corrected rail has no extractable text, the motif exists only in CSS, and both disappear cleanly when a simplified reader drops the stylesheet. The current matrix exercises every page at wide and narrow widths and without CSS, 153 loads altogether. Additional inspection covers light and dark themes, print, mobile reference tables, and the nested answer key. Nine pamphlets remain; their catalogue receives its own opening afterward. |
