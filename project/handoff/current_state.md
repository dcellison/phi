...
| Shelf visual identities | In progress; primer, texts, and manual complete, thirteen pamphlets complete | The pamphlet workbook frame now reaches 143 pages. A waveform and its one dot identify Spoken Punctuation. A pen and reading frame identify Tengwar Mode. The dual-script renderer turns every Tengwar fence into a semantic figure with its glyph line, romanized Phi, gloss, and translation when the source provides one. The seven unsectioned lessons keep their source shape. The exercise page recovers six visible divisions; the last page presents twenty-two numbered writing specimens. Source shapes and list starts remain pinned. Rails contain no extractable text, motifs exist only in CSS, and simplified readers receive clean prose without the stylesheet. The current browser matrix opens every treated page at 1440px, at 390px, and without CSS: 429 loads. Dark and print received separate checks. Direct inspection covered both openings and the densest lesson, answer, error, and reference forms. Ternary numerals is the last workbook; the shelf catalogue follows it. |
