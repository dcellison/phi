...
This snapshot was first assembled after pull request #418 merged on 17 July 2026 and has been refreshed through D053 and the twelfth SEM-09D literary retrofit. Its counts describe the current tracked work; Git history remains the source for branch and merge identifiers.

## Active execution sequence

The roadmap's current sequence is not an invitation to choose among several equal tasks. It records one active path:

1. SEM-09D: complete the final literary pass through *News from Nowhere* chapters 1-3.
...
SEM-09D is in progress. Twelve literary passes are complete through *The Velveteen Rabbit*. *News from Nowhere* chapters 1-3 remain last because their length and internal dependencies make them the capstone of the shelf pass. Active Phi in the book follows, from the cold open through the drafted chapters in file order; the manual, primer, pamphlets, and current documents come after it. A Phi passage changes only when the current form is more faithful or natural there. A transparent composition stays when its parts still teach or express the thought better, and every changed sentence and dependency receives validation.
