...
SEM-09D is in progress. Fourteen literary passes are complete through *News from Nowhere* chapter 2. Chapter 3 remains last because the three chapters share river language, characters, and social misunderstandings. Active Phi in the book follows, from the cold open through the drafted chapters in file order; the manual, primer, pamphlets, and current documents come after it. A Phi passage changes only when the current form is more faithful or natural there. A transparent composition stays when its parts still teach or express the thought better, and every changed sentence and dependency receives validation.
