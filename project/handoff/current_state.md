...
| Shelf visual identities | In progress; primer, texts, and manual complete, pamphlet prototype complete | *The Three Slots* now supplies the pamphlet workbook prototype across its thirteen readings. Its page rail, section labels, responsive tables, interlinear examples, error clinics, exercises, answers, and appendix make the different kinds of work easy to find. A strict manifest pins every source shape and the exercise lists' continued numbering. Twenty-six desktop and phone loads covered the whole pamphlet; opening, table, error, exercise, dark, and print views received separate inspection. A browser reading mode then exposed one condition that matrix had missed: it may discard the site's CSS. The motif lives entirely in CSS, and the compact rail keeps full page names in `aria-label` instead of hidden text. Its two-digit labels vanish harmlessly with the stylesheet. The revised matrix checks each page three ways: desktop, phone, and stylesheet-free, for 39 renderings. No pamphlet Markdown changed. Carry the design through the remaining pamphlets, with room for each one's material, and then give the shelf catalogue its own opening. |
