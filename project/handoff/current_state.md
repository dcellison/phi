...
1. Resume SEM-09D at manual Part VI, then continue through the rest of the manual, primer, pamphlets, and current documents.
...
SEM-09D is in progress. Fifteen literary passes complete the shelf, and all three drafted *News from Nowhere* chapters are among them. The Phi book is current through chapter 7. Manual Parts I through V are reviewed without a lexical gap; Part IV uses one explicitly labelled optional Work term, while Part V stays entirely in base and function vocabulary. The remaining active-corpus retrofit resumes at manual Part VI, followed by the rest of the manual, primer, pamphlets, and current documents. A Phi passage changes only when the current form is more faithful or natural there. A transparent composition stays when its parts still teach or express the thought better, and every changed sentence and dependency receives validation.
