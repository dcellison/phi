...
| Shelf visual identities | In progress; primer, texts, and manual complete | The manual now has a treated contents page as well as 145 treated readings. Its open index gives every part a jump point, groups the teaching material under all 24 chapters, and gathers four back-matter pages. Chapter and part titles have one renderer manifest; the compact outline must match it or the build stops. The cross-page audit found that the repository README still described completed work as drafted. The outline carried 126 mismatches; some titles were obsolete, while others differed only in styling. Both maps now state the present structure without duplicating the page-level catalogue. Directional navigation forms one chain across all 145 readings, and all 147 manual HTML surfaces have valid local links and fragments. The visual matrix covered 292 desktop and phone loads, with further checks in dark mode and print. The pamphlet workbook is the next shelf treatment. |
