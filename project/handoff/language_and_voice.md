...
In *News from Nowhere*, the narrator and the future people share the full vocabulary, including optional modules. Any new root keeps the three-syllable lexical ceiling; only a productive name may reach four syllables. A person who recurs carries one Phi onym at every mention under canon's translation-naming rule, and a continuing speech opens one `sha ... sho` frame rather than one per sentence. Derived English is natural English translated from Phi alone. It preserves Phi's grammatical and discourse commitments without borrowing wording, gender, emphasis, or literary order from Morris. Do not render a source idiom through its metaphor's syntax: conversational entry is `koema`-shaped entry, not `pukate` breakage, and every drafted verb's argument structure is checked against its entry's own usage note, because the parser accepts a grammatical sentence that carries the wrong claim. Architectural arches use Work `toreku`; `loriphi` remains the optical rainbow and enters architecture only as an explicit comparison.
