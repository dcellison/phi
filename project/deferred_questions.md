...
Status: In progress. The primer, text, and manual shelves are complete. The full *Three Slots* pamphlet is now the workbook prototype; the other thirteen pamphlets and the shelf catalogue come next.
...
4. **Pamphlets:** Prototype complete. *The Three Slots* tests the practice-book design from its opening through the appendix. This includes the pages most likely to expose a weak design. A direct page rail and related forms for lessons, tables, examples, error clinics, exercises, answers, and reference material tell readers where they are and what kind of work the page asks of them. The renderer preserves the source's continued exercise numbering, links the opening map to the live pages, and stops the build when either the page set or a recorded source shape changes. The visual check covered all thirteen pages at desktop and phone widths, followed by dark-mode and print checks. The rest of the pamphlet shelf and its catalogue come next.
