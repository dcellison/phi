...
Status: In progress. The primer and text shelves are complete. The manual treatment now covers Parts I through VI.
...
3. **Manual:** In progress. All 128 teaching pages in Parts I through VI now use the working-reference treatment. The source files remain plain Markdown. First Light carries a sunrise and sprout, Soul a heart and sprout, and Phonology a voice and ear. Grammar has its layered mark; Complex Structures has a link and braces; Mastery has a compass and sprout. The shared frame names the part, chapter, and page position. It numbers substantial sections and offers a local map when a page has more than one section. Short introductions and unsectioned essays do not receive an empty map. Speaker rows and responsive reference tables support the pages that need them. Grouped examples, exercise sequences, transmutation steps, and the `rena` syntax frame keep denser material legible without turning the manual into a dashboard. Structural signatures record the block shapes the renderer depends on. Renderer metadata supplies meaningful headings to the one source table that intentionally has none. Browser checks cover every treated page at desktop and phone widths, with representative intermediate-phone, dark, and print checks. Part VII and the back matter come next. After that, review the contents and read across the finished manual for consistency.
