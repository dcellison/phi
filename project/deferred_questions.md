...
Status: In progress. The primer and text shelves are complete. The manual treatment reaches its full 145-page reading order; the contents and final consistency read remain.
...
3. **Manual:** In progress. The working-reference treatment now runs through the colophon, and the source files remain plain Markdown. One frame orients the seven parts without erasing their different marks. Part VII opens an indexed reference desk. Its eight module guides leave every table and example intact, and each long page has a local map. The appendices use a quieter file-and-list mark. Appendix A turns 28 prose definitions into a scan-friendly grid and keeps the longer explanations for all 67 Leipzig abbreviations. A heart beside a writing quill closes the colophon; its manual and footer-linked copies share one rendered body. Structural signatures protect every page in the reading order. The browser opened the full set at desktop and phone widths for 290 checks, followed by dark and print views. That examination caught two long repository paths before publication. The contents page and a read across the whole manual are the remaining work.
