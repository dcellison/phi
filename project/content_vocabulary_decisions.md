...
| `CV-VALUE-01` | importance, priority, merit, deservingness, dignity, and worth | Compositional | base value and worth vocabulary with Work priority where needed | `sone`, `rolia`, `noetha`, `porethu`, `meropi`. Valuing is an act, worth is a claimed property, and priority orders attention under a purpose. Merit and deservingness can state the relevant conduct, contribution, need, criterion, or responsibility instead of ranking a whole person. |
