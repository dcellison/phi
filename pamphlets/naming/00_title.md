# How Phi names people

## A practice companion to the manual

A Phi name is an atom announced by `ne`. It may be an ordinary content word someone bears (*warm*, *joy*, *seed*), a single Phi-shaped name someone chooses without adding it to the dictionary, or a bounded guest/exact form. The doctrine belongs to the manual and canon, while the external-register pamphlet teaches the two boundaries in full.

Two skills interleave throughout. The first is mechanical: placing `ne`, `kona`, and the honorifics in their fixed order, at speaking pace, without thought. The second is the one Phi actually cares about: telling the truth with them: announcing only the relationships you have, resting a name only where presence has earned it, and never once making a person taller than the words around them.

By the end of this pamphlet, you will:

- Hear the difference between a word and a name at the speed of a single particle
- Assemble and validate a personal two- or three-syllable Phi-form name without inventing a dictionary meaning for it
- Introduce yourself with the `nomei` formula, introduce someone else, and ask a name back
- Call to someone by name, by role, or with nothing but the call itself
- Choose among `sa`, `ni`, and `le` honestly, or decline all three without coldness
- Read the household register as one optional use of established-reference omission
- Write names in a language with no capital letters: in romanization, in gloss lines, in English narration, and in Tengwar, where there was never a case to lose
- Choose among a productive onym, a bounded guest form, and an exact source form

This pamphlet assumes the particle system (manual ch9) and the mindful sentence (ch10). It is a companion to manual chapters 21 and 24 and to the canon rulings *`ne` licenses a name atom*, *Productive Phi-form onyms*, *Letters*, and *The external register*.

---

**Contents:**

1. Three kinds of name atom
2. The announcing `ne`
3. Calling: `kona`
4. The three honorifics
5. Established names and the household register
6. The spoken capital
7. The open door
8. Common errors
9. Exercises
10. Appendix: quick reference

---

*Phi does not require a name to become vocabulary. It announces that a proper designation is coming, then lets the bearer supply its form.*
