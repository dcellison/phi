...
*For decades, the conlang community's handshake has been the same: show us your Babel text. Genesis 11:1-9 is the story of how one speech became many, and nearly every constructed language has carried it, a convention Jeffrey Henning proposed as a standard in 1996. Phi's ground truth is the King James Version, the community's usual reference text, stored verbatim and cited verse by verse. Two of Phi's words were coined here: `haluma` (language) and `thiwera` (scatter). It is a transmutation, not a translation. Where the King James has the LORD descend, judge, and scatter, this teller gives those acts to `muila`, the earth itself, and reads the scattering as sowing. Every verse is cited below, departures included, so the distance between the tellings can be measured exactly.*
