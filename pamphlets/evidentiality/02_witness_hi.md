...
Since unmarked speech already implies ordinary firsthand confidence, `hi` is never grammatically required. It is *spent* — on the occasions when witnessing is the point:
...
But the wet ground is not the rain, the empty room is not the leaving, and the trail in the earth is not the snake. The moment your claim outruns your senses by even one step of reasoning, `hi` becomes a false pedigree and `ke` is the honest particle. The elder at the well models the discipline exactly — sees the trail, sees no snake, and says `ke`, never `hi`:
