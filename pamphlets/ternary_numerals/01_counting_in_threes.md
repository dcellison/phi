...
| `wi` | 2 |
| `ta shao` | 3 — one group |
| `ta shao ta` | 4 — one group and one |
| `ta shao wi` | 5 — one group and two |
| `wi shao` | 6 — two groups |
| `wi shao ta` | 7 — two groups and one |
| `wi shao wi` | 8 — two groups and two |
...
Alongside `mu` stands the quantifier `mawha` (none) — `mawha suliwa.` (no snake at all) — and the difference is register: `mu` counts, where `mawha` declares. The well story ends with `mawha suliwa` because the household is not tallying snakes; it is pronouncing the well clear.
