...
Punctuation is where the mode stops needing new marks almost entirely, because Phi already moved its punctuation into speech. The comma of address, the question mark, the quotation marks: all of them are words (`kona`, `wa`, `sha` and `sho`), and a word writes the same in Tengwar as anywhere else. Only two marks in the whole mode are silent, and both exist because a silent mark is justified exactly where no word is standing there to do the work.

| Mark | Tengwar form |
|---|---|
| word separator | a raised dot, optional |
