# rules

* plath focuses on soft sounds
  * like wind or water
  * emphasis on unvoiced sounds
* written in lower case
* all words
  * start with consonant
  * end with a vowel
* emphasis is on the penultimate syllable
* subject object verb sentence structure
  * e.g. i you love
* no s/o articles on personal pronouns
* plural particle comes after s/o particles
* adverbs follow verbs
  * drive carefully
  * adverbs are "-ly" words
* adjectives follow nouns
  * flower blue
* relative clauses come before nouns?
* only object pronouns
  * me, you, it
* verbs are two or three syllables
* nouns are three or four syllables
* no consecutive duplicate vowels
* no articles on pronouns
* no gendered pronouns
* no grammatical gender
* no irregular verbs
* no diacritics
* no shwas
  * e.g. mel spidermn
* all vowels except "a" are spanish pronunciation
  * "a" is pronounced as the english short "a"
    * the open front unrounded vowel [a]
  * optionally use the open back unrounded vowel [ɑ]
    * would function as a slight accent
* phonemic diphthongs are pronounced as separate letters
  * they can be run together when speaking quickly
  * but they remain distinct sounds
* "t" is french [t]
  * voiceless plosive
  * pronounced with tongue against upper teeth
  * not further back, as in english
* plath makes heavy use of voiceless digraphs
  * [ɬ] an uncommon "ch" sound
    * voiceless alveolar lateral fricative
    * pronounced something like "ich" in german
      * but softer, more liquid sounding
    * never occurs at the start of a word
  * [ɸ] a standard "ph" sound
    * voiceless bilabial fricative
  * [ʃ] a standard "sh" sound
    *voiceless postalveolar fricative
  * [θ] a standard unvoiced "th" sound
    * voiceless dental fricative
    * voiced "th" [ð] never occurs in plath
  * [ʍ] a standard "hw" sound
    * voiceless labialized velar approximant
    * voiced "wh" [w] never occurs in plath