...
| D010 | Experimental | Treat practice and domain modules as non-grammatical profiles over shared core resources. | Household and Daily Life, Medical and Bodily Care, Systems and Shared Infrastructure, Commons and Collective Governance, the shared-core Philosophical Reasoning profile, and the cross-domain Accessibility and Participation profile organize existing roots without moving them from the lexicon. Profiles add no grammar or parser mode; any future term remains subject to the ordinary evidence-led admission process. Test all six before treating the format as a durable extension policy. |
