...
The [Work, Craft, and Repair profile](../modules/work_craft_and_repair.md) established 28 optional terms in its first pass for tasks, projects, progress, blocked status, deadlines, schedules, assignment, coordination, supervision, handoffs, skills, competence, expertise, qualification, training, assessment, certification, design, testing, quality, workload, drudgery, exploitation, stock, inventories, supply, offcuts, and provenance. Later source work added `torali` tower, shared `mophiku` shear, and shared `thaweno` reward. `winora` plan and `hasoru` pending belong in base vocabulary.
