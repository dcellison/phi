...
| Relative clause | `whu` and the relative clause before an explicit head noun, or in noun-phrase position when headless | All current `whu` spans were screened. An oblique preposition remains before its gap; the later head noun is outside the relative clause and is not a postposed prepositional object. |
