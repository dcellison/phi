...
The ground truth is the complete 1948 English Article 1 stored in `texts/sources/udhr_1948.txt`. Two aligned units preserve both source sentences. Four optional roots keep distinctions the declaration needs: `sherelo` is a normative right that can survive denial, `remori` is reason as a faculty, `sharino` is conscience as moral self-appraisal, and `naseru` states the final obligation. D108 certifies the result under the isolated process: the two citations reconstruct all 170 normalized source characters, and independent source-blind readers agree on the structure and meaning of both units.
...
| Conduct and modality | Bare necessity would overstate English *should*, while the original unframed manner phrase could attach either to the action or to possession of the obligation. | `naseru phelu` states the obligation. A following `ha naseru tha ... tho phelona` clause gives its required conduct an audible boundary, so reciprocal `wiso` and `roe lomea phiora` belong unambiguously to the action. |
...
The ground truth is KJV Genesis 11:1-9 stored in `texts/sources/kjv_genesis.txt`. Nine aligned units carry all nine verses in order, and ten citations reconstruct all 1,191 normalized source characters. Commons `karami` and Work `noraku` and `torali` are the optional roots that the source needs; every other pressure resolves through base composition or an adjacent source identity. D109 certifies the final stream at SHA-256 `2cb3d2ccc6b5bb33a1860a6081e45842ee0911a6bfa40637e410185e859da136` after two source-blind derivation rounds and an isolated Unit 9 retry.
...
| Event and speech boundaries | The inherited page split simultaneous events and repeated the attribution inside two continuing speeches. | `whau` relates journey and discovery. Each speech now has one quote frame, while the two source accounts of scattering remain separate assertions. |
| Surface and relative roles | Anatomical `menoa` treated land as a human face, while three transitive relatives left their missing role open. | Physical `leko` names the earth's surface. Brick-connecting material is a compound, the authority-holder is established in a complete assertion, and the tower's top receives its own sentence. |
...
| Coverage | Every KJV clause must belong to one aligned unit. | Ten ordered citations reconstruct Genesis 11:1-9 exactly, the continuous Phi reading matches the detailed blocks, and all nine units have source-blind English. |
...
The complete Williams story is a translation with 429 exact source fragments across 426 aligned Phi units. The citation stream reconstructs the stored story exactly and in order. Optional vocabulary enters through six modules, with Household vocabulary doing most of that work.
...
D111 applies the D102 process to all 426 units. The source-to-Phi phase consolidates the two fragile-person citation fragments into one unit and changes 251 final units. Six stable onyms keep recurring participants audible. A complete order sweep puts ordinary temporal and locative adjuncts after explicit subjects, while focused repairs settle past conditional narratives, purpose frames, participant roles, embedded questions, the fever's change, the Rabbit's body discovery, and the final identity proposition. Anonymous English exposes three missing plurals in other things, mainsprings, and eyes. Its final comparison also removes ambiguous relative, temporal, and pronoun boundaries, restores the lining of the ears and Timothi's identity, and brings the healer's instructions, Tupiwa's boast and thought, the moving fern, and the different bedroom back into the Phi layer. All 426 final units pass the sentence parser before the stream freezes at SHA-256 `16597a3af79e10b9ff4e66638108da09e4c9fb6bce6ccd4d275ccb4b841c3f59`.

The primary source-blind context derives all 426 initial units and identifies one unsupported anatomical reading. An independent source-blind audit exposes further order, tense, attachment, and identity faults in the Phi. Their English layers are discarded. After the final source-side repair, two fresh source-blind contexts rederive and independently audit 91 changed units plus the English-only stocking-opening retry. The final detailed and continuous Phi streams agree, every exact gloss follows the lexicon scaffold, and the 429 Williams citations retain all 20,296 normalized characters. No root, module membership, registered compound, function word, or grammar is added.
...
| `texts/velveteen_rabbit.md` | Translation | Certified D111 | The 429 Williams fragments align across 426 Phi units. The final stream freezes at `16597a3a...41c3f59`; fresh source-blind contexts derive every final English layer and independently audit the retry sets. |
