...
|---|---|---|
| Effort and attempt | Replace generic `theula wha shia to phoa` with `shia theula mesatu` | The older phrase said only "all that it did." The event noun of base `mesatu` now says "all its attempts" and leaves completion and success open until `whuo lureko` reports the fruit. |
...
| Greediness on the family round | Replace `pula` with `rinu` | `pula` presents an outcome the person would welcome. `rinu` is ordinary wanting, while `tha ... tho` holds the complete action of taking many things among the families. That action carries the excess present in this scene. |
