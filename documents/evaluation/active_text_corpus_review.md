...
D101 reread the fable's four layers in one working view. Its source-to-Phi findings remain accepted: seventeen non-overlapping citations reconstruct all 1,172 normalized source characters exactly, and the revised Phi uses `whau` for the ongoing dispute, one frame for each continuing speech, `tha ... tho` for the Sun's proposed agreement, explicit coordination for linked actions and the moral contrast, canonical manner and adjunct order, and plural shoulders. D102 rejects the mixed production method. The current parenthetical lines therefore carry no isolation certification and will be discarded before a fresh Phi-only derivation. No root, module membership, registered compound, or grammar was added.
...
| `texts/north_wind_and_sun.md` | Translation | Isolation pending | D101 settles the 17-unit Phi and source partition; D102 requires its parenthetical English to be discarded and derived again from a Phi-only view. |
| `texts/metta_sutta.md` | Translation | Isolation pending | Full source-fidelity pass complete; two optional roots used where their distinctions matter. |
| `texts/solarpunk_manifesto.md` | Translation | Isolation pending | The 38-unit fidelity pass changes 17 units, aligns a 244-form Phi reading with the complete licensed witness, and uses 33 optional roots across all eight modules without altering a source line. |
