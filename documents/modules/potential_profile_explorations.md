...
**Existing resources to test.** `theama` care, `naphe` help, `panoru` support, `phowe` share, `loa` give, `howela` receive, `phelona` require, `kanu` choose, `naweri` refuse, `thekao` acknowledge, `seru` commit, `therilu` rest, `laenu` commons, and `sila` community already cover many concrete relations.
...
**Existing resources to test.** `nuora` food, `nuola` eat, `napa` bread, `phialu` water, `pharelo` cook, `thepalu` garden, `kosha` field, `thinoe` seed, `thuroa` grow, `sholei` gather, `lumera` nourish, `phanoi` portion, `kumoli` grain, `phelu` hold, `nomira` collect, and `phowe` share provide a substantial starting inventory.
...
**Existing resources to test.** `womu` home, `shuroa` shelter, `moru` wall, `toru` roof, `ponu` door, `wira` window, `tomae` floor, `ruela` path, `shoeka` route, `lorima` build, `shiroka` repair, `tenoa` tool, `keli` device, `sulae` warm, `neri` cool, `shurano` safe, and `laenu` commons cover ordinary objects and broad conditions.
...
**Existing resources to test.** `phelui` word, `reo` meaning, `lilea` clarify, `shorela` rephrase, `shekoi` specify, `shelao` summarize, `shane` tell, `thekiro` write, `theo` read, `haolu` speak, `shelu` book, the evidentials, productive names, and `hasha ... hasho` and `patha ... patho` provide the main current mechanisms.
...
**Existing resources to test.** `wisola` exchange, `piru` trader, `loamira` gift, `loa` give, `howela` receive, `phowe` share, `riola` labor, `seru` commit, `phelu` hold, `laenu` commons, `temu` guild, `thua` fair, and `theloma` justice can describe concrete transfers and evaluations but do not conventionalize most economic categories.
...
**Existing resources to test.** `masue` measure, `kiro` precise, `sharoi` exceed, `thenoi` fall short, `siru` marker, `somela` observe, `sano` know, `nohero` believe, `hi` direct evidence, `ke` inference, `ti` report, `ho` assumption, and `patha ... patho` exact material already support a substantial evidence-and-carriage layer.
