...
|---|---|---|
| Process, form, and relation | `keli` device, `tenoa` tool, `teru` process, `norae` method, `norelu` form, `ruka` complex, `whoa` whole, `luwi` flexible, `tinako` rigid, `tekari` straight | Artifacts, methods, forms, and their room for adjustment can be discussed without automatically asserting a system analysis. |
