...
|---|---|---|
| Process, form, and relation | `keli` device, `tenoa` tool, `teru` process, `norae` method, `norelu` form, `ruka` complex, `whoa` whole, `luwi` flexible, `tinako` rigid | Describe artifacts, methods, forms, and their room for adjustment without automatically asserting a system analysis. |
...
| Dimensions and separation | `ponalu` size, `waleru` length, `hirawo` distance, `raeli` height, `lonai` width, `nusho` depth, `masue` measure | A design or record can identify its magnitude while units and specifications stay with the source. Large, long, near, and far remain later judgements. |
...
| Material phase | `muralo` material, `patoku` solid, `larewu` liquid, `heshowa` gas, `kelu` become | An input or stored material can be classified by ordinary phase before a technical model adds exact conditions. |
...
- [The wheel, the chisel, and the promised repair](../evaluation/philosophical_test_corpus.md#15-the-wheel-the-chisel-and-the-promised-repair) can use `nilaki`, `kisholu`, `kiphira`, `thonuki`, and `koluri` while keeping inspection, repair, specification, demonstrated function, approval, and safe return to use distinct.
