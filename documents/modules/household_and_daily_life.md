...
| Size, weight, shape, and fit | `ponalu` size, `waleru` length, `hirawo` distance, `raeli` height, `lonai` width, `nusho` depth, `pamolu` weight, `tekari` straight, `masue` measure | A box can be compared with its shelf, two baskets by weight, cloth with the place it must cover, and furniture with the passage to its room before any exact source value is needed. A cord or edge can be called straight without confusing shape with a direct route. |
...
| Cloth, rest, and temperature | `wetha` cloth, `liru` fabric, `wethalu` garment, `wosanu` absorb, `nulae` sleep, `therilu` rest, `sukaro` hot, `sulae` warm, `wuloe` soft, `kurathi` dry, `wirua` wet | Cloth can take in water without making absorption a household-only action, and bodily conditions remain available beyond domestic use. |
...
B: lo mia ha wonepa shiroka.
PL 1SG PROX table repair.
(We repair this table.)

A: nuora mua wonepa nai.
food LOC table be.
(Food is on the table.)

B: lo mia nuora nuola.
PL 1SG food eat.
(We eat food.)
