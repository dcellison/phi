...
| Upkeep and repair | `sawa` wash, `hewe` wipe, `hiso` clean, `shumeko` sticky, `muhena` dust, `phuri` brush, `pukeri` damage, `rohemi` wear down, `shiroka` repair, `talome` restore, `tenoa` tool, `thinu` put, `phelu` hold | The core can identify sticky residue, distinguish damage from gradual wear, and describe the work that follows. |
