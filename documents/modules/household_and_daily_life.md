...
| Size and fit | `ponalu` size, `waleru` length, `hirawo` distance, `raeli` height, `lonai` width, `nusho` depth, `masue` measure | A box can be compared with its shelf, cloth with the place it must cover, and furniture with the passage to its room before any exact source value is needed. |
