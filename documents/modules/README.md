...
Every module-oriented word follows the complete [word-creation protocol](../../project/development_protocol.md#word-creation-protocol). Its entry uses the target prose contract and bounded semantics, passes phonology and collision checks, supplies structured examples, and keeps generated references in step. Corpus evidence may inspire or improve a word, but a personal language does not need an external burden of proof before its maintainer coins a useful or valued concept.
...
The [Module overlap assessment](module_overlap_assessment.md) accounts for the 227 roots established during the eight original vocabulary passes. Seventy-seven of those have shared membership, 137 keep one module, and thirteen belong in base vocabulary. The [Base vocabulary placement assessment](base_vocabulary_placement_assessment.md) then checks the other 819 content roots and classifies thirty older roots where their lexical center is specialized. Later connected reviews added `hoshemi` hypothesis, nine Systems roots, three Ecological parameter nouns, Commons `kowanu` governance and `pashenu` petition, the Metta Sutta distinctions `serathi`, `taleri`, and `wurashi`, Philosophical `remori` reason, Work `torali` tower and `toreku` arch, shared Ecological and Work `mophiku` shear, and the On Giving distinctions `parelu` deserve and `thaweno` reward. Ecological `lomathu` now distinguishes manure from bodily waste, while `tomewu` urine and `mokathi` faeces have moved from base study into Medical and Bodily Care. Gibran's grain sequence adds `thapori` thresh, `shireku` sift, `napewa` husk, and `pumeli` knead across the three material learning paths. The current optional layer contains 277 roots and 419 memberships.

## Active Profiles

| Profile | Status | Module roots coined here | Current memberships | Base vocabulary established here | Purpose |
|---|---|---:|---:|---|---|
| [Household and Daily Life](household_and_daily_life.md) | **Established vocabulary module** | 30 | 47 | `mirewu` private | Domestic relations, storage, cleaning, cloth, meals, food preparation, and household energy. |
| [Medical and Bodily Care](medical_and_bodily_care.md) | **Established vocabulary module** | 30 | 54 | `lesawi` consent | Symptoms, findings, health conditions, clinical evaluation, intervention effects, illness course, transmission, and public health. |
| [Systems and Shared Infrastructure](systems_and_shared_infrastructure.md) | **Established vocabulary module** | 33 | 50 | `phelure` store | System structure, inputs and outputs, electricity, control, feedback, technical state, upkeep, reliability, hazards, and specifications. |
| [Philosophical Reasoning](philosophical_reasoning.md) | **Established vocabulary module** | 27 | 30 | `sherewa` claim, `thesori` evidence, `remotha` reason, `kirema` criterion | Argument structure, evidence, hypotheses, inference, validity, definition, epistemic stance, rights, conscience, deservingness, reward, and competing values. |
| [Accessibility and Participation](accessibility_and_participation.md) | **Established vocabulary module** | 30 | 46 | `sowelu` disabled | Access relations, communication forms, timing, technical fit, support, exclusion, and advocacy. |
| [Commons and Collective Governance](commons_and_collective_governance.md) | **Established vocabulary module** | 29 | 55 | `sirami` record, `kawhera` coerce, `whepelo` retaliate | Membership, institutions, governance, authority, rights, deservingness, petitions, collective procedure, allocation, accountability, records, review, and institutional harm. |
| [Ecological Systems and Material Life](ecological_systems_and_material_life.md) | **Established vocabulary module** | 30 | 60 | `howenu` weather, `muralo` material, `panuri` resource | Ecological organization, land and water processes, environmental parameters, weather and climate, pollution and exposure, material afterlives, energy, and repeated ecological observation. |
| [Work, Craft, and Repair](work_craft_and_repair.md) | **Established vocabulary module** | 28 | 77 | `winora` plan, `hasoru` pending | Work units and status, coordination, skill and qualification, training and evaluation, labor conditions, reward, design, built forms, electrical work, and material traceability. |
