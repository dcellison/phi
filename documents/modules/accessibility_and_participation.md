...
The established [Commons and Collective Governance module](commons_and_collective_governance.md) supplies `wemakori` member, `shanelori` represent, `silathora` role, `loathora` delegate, `kanorami` authority, `kanuloa` authorize, `norathume` procedure, `kanusholu` decision, `mawokelu` abstain, `shaneweli` notify, `siranomi` record, `turekiro` review, `whekatuma` appeal, `kanuwhera` coerce, and `whekapelo` retaliate. These words can identify governance relations around access, but authorization is not consent, notification is not accessible receipt, representation is not faithful interpretation, procedure is not participation, and a decision does not establish that access was achieved.
...
| Ask before assisting | Use a polite whole-clause question with `pi wa`, state the proposed action, and mark possibility with `po` where appropriate. | `po` also expresses practical possibility. The construction does not state scope, duration, revocation, or whether the helper would gain representational authority. |
...
| Permission, refusal, and acknowledgement protect autonomy across domains | **Recurring** in bodily care and both access scenarios | Retain these resources in shared core and test their scope before proposing any consent-oriented addition. |
...
| Timing and cognitive access | processing time, predictable sequence, break, postponement, decision deadline; reuse `shaneweli` for notice while stating how far in advance it occurs | Repeated cases where `phoe`, `hasi`, `therilu`, `thilonu`, and notification still fail to distinguish the relevant timing relation. |
| Support and authority | accommodation, assistance, decision support, interpreter role, proxy; reuse `shanelori`, `silathora`, `loathora`, `kanorami`, and `kanuloa` for representation and authority where their definitions fit | Independent cases contrasting help, faithful interpretation, speaking for someone, and deciding for someone. |
...
| Identity, testimony, and harm | disability, impairment, access identity, discrimination, denial of access, coercive disclosure; reuse `kanuwhera` for coercion where its definition fits | First-person or source-faithful scenarios reviewed for whether a core paraphrase erases identity, agency, responsibility, or harm. |
