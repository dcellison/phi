...
| Movement and safety | `ponu` door, `ruela` path, `shoeka` route, `koema` enter, `wepu` go, `losha` wide, `hieru` narrow, `selawi` slippery, `shurano` safe, `pukea` danger | The environment can be described before the access relation is named. This includes a surface that gives too little traction for the expected movement. |
