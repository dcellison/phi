...
| Movement and safety | `ponu` door, `ruela` path, `shoeka` route, `koema` enter, `wepu` go, `ponalu` size, `waleru` length, `hirawo` distance, `lonai` width, `losha` wide, `hieru` narrow, `selawi` slippery, `shurano` safe, `pukea` danger | The environment can be described before the access relation is named. A route's length, a doorway's width, or the distance to a service can matter without proving that the route is usable. |
