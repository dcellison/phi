...
| Philosophical capability | **PARTIAL** | Matrix, repair conventions, and twelve test dialogues exist; several operations remain gaps. |
...
| Practice and domain profiles | **PARTIAL** | An experimental contract, four domain profiles, one shared-core practice profile, one cross-domain accessibility profile, and a recorded proposed-profile catalogue exist. The format still needs use evidence. |
| Original Phi corpus | **PARTIAL** | Twelve philosophical dialogues exist; sustained essays and spontaneous transcripts do not. |
...
| PHL-02 | **DONE** | Twelve original regression dialogues covering ecological tradeoff, repair/replace, evidential revision, external-language critique, definition under pressure, two measurement/systems challenges, bodily-care decision-making, two access cases, commons allocation, and shared-infrastructure accountability. | `documents/philosophical_test_corpus.md` | Existing dialogues remain validated and revisable. |
...
| PHL-05 | **PARTIAL** | Test compositional operations: cause versus reason, retraction, competing goods, responsibility, uncertainty, and suspended judgment. | Competing-goods and responsibility cases now exist in scenarios 11 and 12; other operations and transformations remain | Transform the commons scenarios and record where priority, responsibility scope, and provisional acceptance are lost in paraphrase. |
| PHL-06 | **READY** | Test current gaps: logical validity, contradiction, consistency, counterexample, systems, feedback, and emergence. | Gap rows in the matrix | Use exact terminology plus transparent Phi analysis in several texts before proposing core forms. |
...
| SEM-01A | **PARTIAL** | Test practice and domain profiles as shared-core, non-grammatical organization. | Module contract; Household and Daily Life; Medical and Bodily Care; Systems and Shared Infrastructure; Philosophical Reasoning; Accessibility and Participation; Commons and Collective Governance | Run each profile's next-use test before treating the format as a durable extension policy. |
| SEM-01B | **PARTIAL** | Evaluate the recorded proposed-profile catalogue. | `documents/modules/README.md`; Accessibility and Commons moved to active after satisfying their gates | Add independent ecological scenarios before creating Ecological Systems and Material Life; audit craft vocabulary and write a work-sharing scenario before creating Work, Craft, and Repair. |
| SEM-02 | **PARTIAL** | Map weak modern domains: technology, science, government, law, work, disability, reproductive health, sexuality, and institutions. | Assessment, survey, two access scenarios and profile, two commons scenarios and profile | Test accessibility outside meetings and a standing institution outside councils, then add ecological scenarios while keeping sensitive domains scenario-gated. |
| SEM-03 | **PARTIAL** | Solarpunk philosophical scenario corpus. | Original regression corpus now covers ecology, energy, care, consent, accessibility, commons allocation, collective decisions, and accountability | Continue with mutual aid, institutional failure beyond one council, and conflicting visions of a good future. |
...
| COR-02 | **DONE** | Twelve original philosophical regression dialogues. | `documents/philosophical_test_corpus.md` | Preserve as baseline tests rather than polished doctrine. |
