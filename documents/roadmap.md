...
| SEM-01C | **NEXT** | Revisit active Phi texts after the current module set and classification reviews are complete. | All eight active profiles, the overlap assessment, and the reverse base-placement assessment are complete | Inventory every active scenario, dialogue, story, teaching text, and translation; update wording where module vocabulary makes the meaning clearer or less evasive, and explicitly preserve deliberate before-state evidence. |
