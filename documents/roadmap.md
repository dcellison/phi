...
| Evidence-led semantic growth | **PARTIAL** | Seven profiles have completed full vocabulary passes; Household and Daily Life still awaits its pass. |
| Practice and domain profiles | **PARTIAL** | Eight active profiles and a recorded proposed-profile catalogue exist; seven are established vocabulary modules with speaker chapters, while Household remains an organizational profile. |
| Original Phi corpus | **PARTIAL** | Fifteen philosophical dialogues exist; a corpus-wide after-state review is queued after the remaining Household pass and the multi-module membership assessment, while sustained essays and spontaneous transcripts do not yet exist. |
...
| SEM-01C | **PENDING** | Revisit active Phi texts after the current module set and membership review are complete. | Maintainer-directed sequencing: complete Household, then SEM-01D, before revising texts for expanded vocabulary | Inventory every active scenario, dialogue, story, teaching text, and translation; update wording where module vocabulary makes the meaning clearer or less evasive, and explicitly preserve deliberate before-state evidence. |
...
| SEM-02 | **PARTIAL** | Map weak modern domains: technology, science, government, law, work, disability, reproductive health, sexuality, and institutions. | Assessment, survey, seven established vocabulary modules, two access scenarios, and work and care scenarios | Test the Work, Medical, and Accessibility modules beyond their source scenarios, inhabited-place heat and cooling, and a standing institution outside councils while keeping sensitive domains scenario-gated. |
...
| SEM-05 | **DONE** | Complete implementation standard for each new word. | Development protocol and 197 entries across the first seven established modules, plus shared-core `lesawi` consent and `sowelu` disabled | Require full schema fields, bounded scope, semantic contrasts, examples, phonetic checks, appropriate core or module placement, and generated references. |
