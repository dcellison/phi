...
| Original Phi corpus | **PARTIAL** | Fifteen philosophical dialogues exist; a corpus-wide after-state review is queued after the multi-module membership assessment, while sustained essays and spontaneous transcripts do not yet exist. |
...
| SEM-01A | **DONE** | Develop practice and domain profiles as optional lexical organization under one grammar. | Eight established profiles; Philosophical Reasoning has 22 module words, Systems and Shared Infrastructure has 25, and the other six have 30 each, with generated indexing and speaker chapters; shared-core additions are `lesawi` consent, `sowelu` disabled, and `mirewu` private | Revise an established profile through use, or add another only when its organization would help. |
...
| SEM-01C | **PENDING** | Revisit active Phi texts after the current module set and membership review are complete. | All eight active profiles are established; SEM-01D remains the final prerequisite before the corpus review | Inventory every active scenario, dialogue, story, teaching text, and translation; update wording where module vocabulary makes the meaning clearer or less evasive, and explicitly preserve deliberate before-state evidence. |
| SEM-01D | **NEXT** | Assess genuine multi-module vocabulary membership after all current modules are established. | The schema and tooling support a list of modules, but every current module entry has one designation because passes have used a de facto primary-module convention | Distinguish cross-module dependency from equal curricular membership, add multiple designations where speakers of either module should independently learn the word, regenerate indexes, and avoid tagging every merely useful word. |
...
| SEM-05 | **DONE** | Complete implementation standard for each new word. | Development protocol and 227 entries across all eight established modules, plus shared-core `lesawi` consent, `sowelu` disabled, and `mirewu` private | Require full schema fields, bounded scope, semantic contrasts, examples, phonetic checks, appropriate core or module placement, and generated references. |
