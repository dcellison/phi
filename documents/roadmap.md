...
| SEM-01C | **IN PROGRESS** | Revisit active Phi texts after the current module set and classification reviews are complete. | The [active text corpus review](active_text_corpus_review.md) records completed source-fidelity passes for *The North Wind and the Sun* and the *Metta Sutta*, with the remaining literary shelf tracked there | Continue through the literary shelf with the established method, then inventory scenarios, dialogues, and teaching texts; update wording where module vocabulary makes the meaning clearer or less evasive, and explicitly preserve deliberate before-state evidence. |
