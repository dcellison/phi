...
| Original Phi corpus | **PARTIAL** | Fifteen philosophical dialogues exist; the module overlap and base-placement reviews are complete, so the corpus-wide after-state review can begin. Sustained essays and spontaneous transcripts do not yet exist. |
...
| SEM-01C | **IN PROGRESS** | Revisit active Phi texts after the current module set and classification reviews are complete. | The [active text corpus review](active_text_corpus_review.md) records the completed Metta Sutta translation review and paired North Wind translation and transmutation review, with a declared method for every remaining work | Continue through the transmuted shelf: repair accidental loss and misleading back-translations, refresh stale vocabulary, and preserve each deliberate transformation; then inventory scenarios, dialogues, and teaching texts. |
...
| ID | Status | Deliverable | Evidence | Completion or next criterion |
|---|---|---|---|---|
| COR-01 | **DONE** | A validated shelf of two close translations, eleven transmuted works, and extensive teaching examples; one fable appears in both forms. | `pamphlets/`; manual; primer | Keep every rendering labeled by method and continue to distinguish deliberate transmutation choices from Phi's general expressive capacity. |
