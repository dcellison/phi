...
- **Optional lexical modules** (settled 2026-07-10 / Phi 2026.2): Phi has one canonical lexicon and one grammar. A content entry may carry validated `modules` metadata so learners can choose specialized vocabulary without treating it as required general study. Module words obey ordinary Phi phonology, syntax, event and quality noun rules, and the complete vocabulary schema; a module adds no particle, inflection, parser mode, or incompatible construction. Speakers unfamiliar with a module term may ask for a core paraphrase. Philosophical Reasoning is the first established vocabulary module with 22 terms, and Systems and Shared Infrastructure is the second with 25; each of the other six active modules has 30. Every module has a Part VII speaker guide. A profile pass may place a broadly necessary word in shared core rather than optional vocabulary, as Medical does with `lesawi` consent, Accessibility with `sowelu` disabled, and Household with `mirewu` private.
