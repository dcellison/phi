...
- **Optional lexical modules** (settled 2026-07-10 / Phi 2026.2): Phi has one canonical lexicon and one grammar. A content entry may carry validated `modules` metadata so learners can choose specialized vocabulary without treating it as required general study. Module words obey ordinary Phi phonology, syntax, event and quality noun rules, and the complete vocabulary schema; a module adds no particle, inflection, parser mode, or incompatible construction. Speakers unfamiliar with a module term may ask for a core paraphrase. Philosophical Reasoning is the first established vocabulary module with 22 terms, Systems and Shared Infrastructure is the second with 25, Ecological Systems and Material Life is the third with 30, Commons and Collective Governance is the fourth with 30, Work, Craft, and Repair is the fifth with 30, Medical and Bodily Care is the sixth with 30, and Accessibility and Participation is the seventh with 30; each has a Part VII speaker guide. A profile pass may place a broadly necessary word in shared core rather than optional vocabulary, as Medical does with `lesawi` consent and Accessibility does with `sowelu` disabled.
