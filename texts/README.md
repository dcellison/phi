...
[What certified means](../documents/reference/translation_certification.md) explains the mark shown beside translations that have completed the process, including the plain fact that certified work can still contain errors. [How a Phi translation is made](../documents/reference/translation_process.md) gives the full working procedure. The [certification ledger](../documents/evaluation/translation_process_status.md) names every translation document and shows which ones have completed it. CI checks the ledger against the shelf. Adding a translation or changing a certified layer therefore requires an explicit status update.
