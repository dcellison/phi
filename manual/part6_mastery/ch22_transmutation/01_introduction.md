...
The word is borrowed from the old alchemists, and the borrowing is honest: transmutation changes the substance, not just the label. Sometimes what comes out is smaller than what went in: a news article loses its statistics. Sometimes it follows Phi's values more closely: the second case study chooses the structure of nonviolent communication and records that choice instead of pretending grammar made it inevitable.

The text shelf names the method used for each rendering. The Metta Sutta, *lothea thole*, is a close translation: each claim in its chosen English source remains answerable to the Phi. *The North Wind and the Sun*, Schleicher's fable, and UDHR Article 1 set a close translation before a transmutation. The Babel story, the Heart Sutra, and five Tao chapters extend that arrangement across longer sources. On Children makes the comparison inside *The Prophet*, where Gibran's bow and arrow stand beside Phi's tree and seed. In each case the two promises can be compared within one work. The other adapted works are transmutations. Their changes are part of the work rather than defects hidden behind a source citation.
