...
Phi resists automatic word-for-word carriage in three ways. Its analytic, verb-final grammar reorganizes source sentence shapes. Its concept space draws different boundaries: `womu` joins dwelling and belonging, while `kuri` names only the bodily direction covered by one sense of English *right*. Some core vocabulary is absent on purpose. A transmuter may treat those absences as invitations to analyze; an exact reporter uses `patha … patho` when rebuilding would falsify wording, erase identity, or lose necessary precision.
