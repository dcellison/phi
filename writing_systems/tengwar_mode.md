...
- `scripts/build_site.py`: the `tengwar_mode` pamphlet shows every Phi line as a Tengwar rendering directly above its romanization, always visible, since teaching that correlation is the pamphlet's whole purpose. The texts shelf and every other pamphlet carry no Tengwar rendering at all.
