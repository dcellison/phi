...
- Reread every back-translation against the lexicon entry of its own Phi words, not just its citation (see the check in step 6); a back-translation can be a technically correct gloss and still flatten a word chosen for its intensity, or claim a stronger meaning than a plain word actually has, and the validator has no way to catch either direction.
